...
export async function calculateDrivingRoute(origin: LatLngLiteral, destination: LatLngLiteral) {
  ...
  return new Promise<{ distanceKm: number; etaMinutes: number; result?: any }>((resolve, reject) => {
    ...
    directions.route(
      ...
      (result: any, status: string) => {
        if (status !== "OK" || !result?.routes?.[0]?.legs?.[0]) return reject(new Error("Unable to calculate route"));
...
export function googleMapsDirectionsUrl(input: { origin?: LatLngLiteral; destination: LatLngLiteral | string; waypoint?: LatLngLiteral | string }) {
  const destination = typeof input.destination === "string" ? input.destination : `${input.destination.lat},${input.destination.lng}`;
  const origin = input.origin ? `&origin=${encodeURIComponent(`${input.origin.lat},${input.origin.lng}`)}` : "";
