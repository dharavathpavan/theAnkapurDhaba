...
import {
  ...
  googleMapsDirectionsUrl as buildDirectionsUrl,
  hasGoogleMapsKey,
  loadGoogleMaps,
  type LatLngLiteral,
...
export function DeliveryMap({
  ...
  const destinationCoords = coordsFrom(
    order.delivery?.destinationLat,
    order.delivery?.destinationLng,
  );
  const destination =
    order.customer.address || order.delivery?.destinationText || "Customer destination";
  ...
  const progress = Math.max(
    0,
    Math.min(1, order.delivery?.routeProgress ?? progressFromStatus(order.status)),
  );
      ...
      .then((google) => {
        ...
        const map =
          ...
          new google.maps.Map(mapRef.current, {
            center: riderCoords ||
              restaurantCoords ||
              routeDestination || { lat: 18.7283, lng: 78.4477 },
            zoom: 14,
...
function Marker({
  ...
  return (
    ...
    >
      ...
      <div className="mt-1 rounded-full bg-white px-2 py-0.5 text-[10px] font-black text-zinc-700 shadow">
        {label}
      </div>
    </div>
  );
...
export function googleMapsDirectionsUrl(order: Order) {
  ...
  const origin = order.delivery?.currentLocation
    ? { lat: order.delivery.currentLocation.lat, lng: order.delivery.currentLocation.lng }
