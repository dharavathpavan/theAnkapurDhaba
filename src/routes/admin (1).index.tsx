...
export const Route = createFileRoute("/admin (1)/")({
  component: Dashboard,
});
...
function Dashboard() {
  ...
  const storeOpen = customerContent?.store ? (customerContent.store as Record<string, unknown>).isOpen !== false && (customerContent.store as Record<string, unknown>).status !== "closed" : true;
