...
function SupportPage() {
  ...
  const filteredFaqs = useMemo(() => {
    ...
    return faqs.filter((faq) => {
      const blob =
        `${faq.category} ${faq.question} ${faq.answer} ${faq.keywords.join(" ")}`.toLowerCase();
      return (
        (!needle || blob.includes(needle)) && (!category || faq.category === category || needle)
      );
    });
  }, [category, faqs, query]);
    ...
    <div className="mx-auto max-w-6xl px-4 pb-32 pt-5 md:px-6 md:py-8">
      <section className="rounded-[34px] bg-zinc-950 p-6 text-white shadow-xl shadow-zinc-950/20">
        <div className="flex items-center gap-3">
          <span className="grid h-14 w-14 place-items-center rounded-3xl bg-red-600">
        ...
        <label className="mt-6 flex min-h-14 items-center gap-3 rounded-3xl bg-white px-4 text-zinc-950">
          ...
          <input
            ...
            placeholder="Search payment, refund, delivery, order..."
        ...
        {categories.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex min-w-fit items-center gap-2 rounded-2xl px-4 py-3 text-sm font-black ${category === item.id ? "bg-red-600 text-white" : "bg-white text-zinc-700 ring-1 ring-zinc-100"}`}
            >
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-[1fr_390px]">
        <section className="space-y-3">
          ...
          {filteredFaqs.length === 0 ? (
            <p className="rounded-3xl bg-white p-5 text-sm font-semibold text-zinc-500">
              No matching answer. Create a ticket and our team will help.
            </p>
          ) : (
            filteredFaqs.map((faq) => (
              <details
                key={faq.id}
                className="group rounded-3xl bg-white p-5 shadow-sm ring-1 ring-zinc-100"
              >
                ...
                <div className="mt-3 text-xs font-black uppercase tracking-widest text-red-600">
                  {faq.category}
                </div>
              </details>
            ))
                  ...
                  tickets.map((ticket) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="rounded-full bg-zinc-100 px-3 py-1 text-xs font-black capitalize">
                          {ticket.status.replace(/_/g, " ")}
                        </span>
                      </div>
                      <div className="mt-3 flex items-center justify-between gap-3">
                        <p className="text-sm font-semibold text-zinc-500">
                          {ticket.category} · {new Date(ticket.updatedAt).toLocaleString()}
                        </p>
                        <button
                          type="button"
                          onClick={() =>
        ...
        <aside className="h-fit rounded-[34px] bg-white p-5 shadow-sm ring-1 ring-zinc-100">
          <h2 className="flex items-center gap-2 text-2xl font-black">
            <MessageCircle className="h-6 w-6 text-red-600" /> Create ticket
          </h2>
          {!isAuthenticated() ? (
            <div className="mt-4 rounded-2xl bg-zinc-50 p-4">
              ...
              <Link
                to="/login"
                className="mt-3 inline-flex rounded-2xl bg-red-600 px-4 py-3 font-black text-white"
              >
                Sign in
              </Link>
            </div>
            ...
            <div className="mt-4 space-y-3">
              <select
                ...
                className="h-13 w-full rounded-2xl bg-zinc-50 px-4 font-bold outline-none ring-1 ring-zinc-200"
              >
                {categories.map((item) => (
              ...
              <textarea
                ...
                className="w-full rounded-2xl bg-zinc-50 p-4 font-semibold outline-none ring-1 ring-zinc-200"
              />
              <select
                value={priority}
              ...
              <label
                className={`flex min-h-14 cursor-pointer items-center justify-center gap-2 rounded-2xl border border-dashed border-red-200 bg-red-50 px-4 text-sm font-black text-red-700 ${uploadingMedia ? "opacity-60" : ""}`}
              >
              ...
              {media.length > 0 && (
                <div className="grid grid-cols-3 gap-2">
                  {media.map((url) => (
                    <div
                      key={url}
                      className="relative overflow-hidden rounded-2xl bg-zinc-100 ring-1 ring-zinc-200"
                    >
                      {isVideoMedia(url) ? (
                        <video src={url} className="h-20 w-full object-cover" muted playsInline />
                      ) : (
                        <img
                          src={url}
                          alt="Attached support media"
              ...
              <button
                ...
                disabled={createTicket.isPending || uploadingMedia || !subject || !description}
                className="min-h-14 w-full rounded-2xl bg-red-600 font-black text-white disabled:opacity-50"
              >
