  ...
  head: () => ({
    meta: [
      { title: "Menu - Ankapur Dhaba" },
      {
        name: "description",
        content:
          "Browse Ankapur Dhaba menu with biryani, chicken, mutton, veg curries, breads and desserts.",
      },
    ],
  }),
  component: MenuPage,
...
function MenuPage() {
  ...
  const categories = useMemo(
    () => [
      "All",
      ...(home?.categories.map((c) => c.name) ?? Array.from(new Set(items.map((i) => i.category)))),
    ],
    [home, items],
  );
          ...
          {filtered.map((item) => {
            ...
            return (
              <MenuCard
                ...
                onAdd={() =>
                  item.sizes?.length || item.addons?.length || item.variantGroups?.length
                    ? setSelected(item)
                    : add(item)
                }
              />
  ...
  return (
    <article className="grid grid-cols-[1fr_112px] gap-3 rounded-2xl bg-white p-3 shadow-sm ring-1 ring-zinc-100 sm:grid-cols-[1fr_132px] sm:rounded-3xl md:grid-cols-[1fr_170px]">
      ...
      <div className="relative">
        <button
          onClick={onOpen}
          className="block aspect-square w-full overflow-hidden rounded-2xl bg-zinc-100 sm:rounded-3xl"
        >
          <img
        ...
        <button
          ...
          className="absolute -bottom-2 left-1/2 flex min-h-11 min-w-24 -translate-x-1/2 items-center justify-center rounded-2xl bg-white px-4 py-2.5 text-xs font-black text-red-600 shadow-lg ring-1 ring-red-100 disabled:text-zinc-400 sm:min-w-24 sm:px-4 sm:text-sm"
        >
    ...
    >
      <div className="absolute inset-x-0 bottom-0 max-h-[92vh] overflow-y-auto rounded-t-[32px] bg-[#F8F9FB] shadow-2xl md:left-1/2 md:top-1/2 md:max-h-[88vh] md:max-w-2xl md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-[32px]">
        <div className="relative aspect-[16/10] bg-zinc-100">
          <img
            src={resolveMediaUrl(item.images?.[0]?.url || item.image)}
            alt={item.name}
            onError={imageFallback}
            className="h-full w-full object-cover"
          />
        ...
        <div className="space-y-5 p-5">
              ...
              {item.sizes.map((s) => (
                ...
                >
                  <span className="font-bold">
                    {s.name}
                    <span className="ml-2 text-xs text-zinc-500">{s.serves}</span>
                  </span>
          ...
          <label className="block">
            ...
            <textarea
              ...
              placeholder="No onion, less spicy, extra masala..."
              className="mt-2 w-full rounded-3xl border border-zinc-200 bg-white p-4 outline-none focus:border-red-400"
              rows={3}
            />
          </label>
          ...
          <button
            ...
            disabled={!item.available}
            className="sticky bottom-4 flex min-h-14 w-full items-center justify-between rounded-3xl bg-red-600 px-5 font-black text-white shadow-xl shadow-red-600/25 disabled:bg-zinc-300"
          >
            <span>Add item</span>
            <span>₹{price}</span>
          </button>
...
function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-black ${cls}`}
    >
      {tone === "veg" && <Leaf className="h-3 w-3" />}
      {children}
    </span>
  );
}
