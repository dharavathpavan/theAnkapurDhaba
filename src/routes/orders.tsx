        ...
        <div className="mt-5 space-y-7">
          {activeOrder && (
            <Link
              ...
              className="block rounded-[28px] bg-zinc-950 p-5 text-white shadow-xl"
            >
          ...
          <OrderGroup
            ...
            orders={current}
            onReorder={(order) => reorder(order, add, navigate)}
          />
          <OrderGroup
            title="Order history"
            orders={past}
...
function OrderGroup({
  ...
  onReorder: (order: Awaited<ReturnType<typeof listMyOrders>>[number]) => void;
}) {
    ...
    <section>
      ...
      <div className="space-y-3">
        {orders.map((order) => (
          <article
            key={order.id}
            className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-zinc-100"
          >
