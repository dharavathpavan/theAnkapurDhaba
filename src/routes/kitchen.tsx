...
function KitchenDisplaySystem() {
  ...
  const visibleOrders = useMemo(
    () =>
      activeOrders.filter((o) =>
        matchesFilters(o, query, typeFilter, priorityFilter, stationFilter),
      ),
  ...
  async function acceptOrder(order: Order) {
    ...
    const updated = await updateOrderKds(order.id, {
      ...
      metadata: {
        acceptedAt,
        priority: meta(order).priority || "normal",
  ...
  async function setStatus(
    order: Order,
    status: OrderStatus,
    metadata: Partial<DeliveryDetails> = {},
  ) {
    const stamps: Partial<DeliveryDetails> = {};
    if (status === "preparing")
      stamps.startedAt = meta(order).startedAt || new Date().toISOString();
  ...
  return (
    <div
      className={`min-h-screen bg-[#121212] text-white ${flash ? "animate-[kds-flash_1s_ease-out]" : ""}`}
    >
      <header className="sticky top-0 z-30 border-b border-white/10 bg-[#121212]/95 px-4 py-4 backdrop-blur-xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs uppercase tracking-[0.3em] text-zinc-400">
                Live sync {syncAge}s ago - touch optimized
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="grid min-h-14 min-w-14 place-items-center rounded-2xl bg-zinc-800 text-zinc-100"
            >
        ...
        <Toolbar
          ...
          onBulkDelay={() =>
            bulk(undefined, { delayReason: "High Load", delayExtraMinutes: 10, managerAlert: true })
          }
          onBulkPrint={() =>
      ...
      <main
        className={`grid gap-4 p-4 transition ${incoming.length > 0 ? "blur-sm" : ""} lg:grid-cols-3`}
      >
        <KdsLane
          title="New"
          ...
          onStart={(o) => setPrepOrder(o)}
          onReady={(o) => setStatus(o, "ready")}
          onDelay={setDelayOrder}
          onCancel={(o) => setStatus(o, "cancelled")}
          onPrint={enqueuePrint}
        />
        <KdsLane
          title="Preparing"
          count={groups.preparing.length}
          ...
          setSelectedIds={setSelectedIds}
          onOpen={setSelectedOrder}
          onStart={(o) => setStatus(o, "preparing")}
          onReady={(o) => setStatus(o, "ready")}
          onDelay={setDelayOrder}
          onCancel={(o) => setStatus(o, "cancelled")}
          onPrint={enqueuePrint}
        ...
        <KdsLane
          ...
          onStart={(o) => setStatus(o, "preparing")}
          onReady={(o) =>
            setStatus(o, "out_for_delivery", {
              readyAt: meta(o).readyAt || new Date().toISOString(),
            })
          }
          onDelay={setDelayOrder}
          onCancel={(o) => setStatus(o, "cancelled")}
      ...
      {incoming.length > 0 && (
        <NewOrderPopup
          ...
          onAccept={() => acceptOrder(incoming[0])}
          onReject={() => setStatus(incoming[0], "cancelled")}
        />
      )}
      {prepOrder && (
        <PrepTimeSheet
          ...
          onSelect={(minutes) => setPrepTime(prepOrder, minutes)}
        />
      )}
      {delayOrder && (
        <DelayDialog
          order={delayOrder}
          onClose={() => setDelayOrder(null)}
          onDelay={(reason, minutes) => delay(delayOrder, reason, minutes)}
        />
      )}
      {soundPanel && (
        <SoundSettingsPanel
          ...
          onClose={() => setSoundPanel(false)}
          onTest={() => playBell(settings, audioRef)}
        />
      )}
      {selectedOrder && (
        <OrderDetails
    ...
    <div className="mt-4 flex flex-wrap items-center gap-2">
      <label className="relative min-w-[260px] flex-1">
        ...
        <input
          ...
          className="h-14 w-full rounded-2xl border border-white/10 bg-zinc-900 pl-12 pr-4 text-lg outline-none focus:border-red-500"
        />
      </label>
      <KdsSelect
        icon={Truck}
        ...
        options={["all", "delivery", "pickup", "dinein"]}
      />
      <KdsSelect
        icon={ShieldAlert}
        value={props.priorityFilter}
        onChange={props.setPriorityFilter}
        options={["all", "vip", "express", "normal", "scheduled"]}
      />
      <KdsSelect
        icon={Filter}
        value={props.stationFilter}
        onChange={props.setStationFilter}
        options={props.stations}
      />
      <button
        onClick={() => props.setBulkMode(!props.bulkMode)}
        className={`min-h-14 rounded-2xl px-5 font-display text-lg tracking-widest ${props.bulkMode ? "bg-yellow-400 text-black" : "bg-zinc-800 text-white"}`}
      >
        BULK {props.selectedCount}
      </button>
      {props.bulkMode && (
        <>
          <button
            onClick={props.onBulkAccept}
            className="min-h-14 rounded-2xl bg-cyan-500 px-5 font-display text-lg text-black"
          >
            ACCEPT
          </button>
          <button
            onClick={props.onBulkReady}
            className="min-h-14 rounded-2xl bg-emerald-500 px-5 font-display text-lg text-black"
          >
            READY
          </button>
          <button
            onClick={props.onBulkDelay}
            className="min-h-14 rounded-2xl bg-orange-500 px-5 font-display text-lg text-black"
          >
            DELAY
          </button>
          <button
            onClick={props.onBulkPrint}
            className="min-h-14 rounded-2xl bg-zinc-700 px-5 font-display text-lg text-white"
          >
            PRINT
          </button>
        </>
  ...
  return (
    <label className="relative">
      ...
      <select
        ...
        className="h-14 rounded-2xl border border-white/10 bg-zinc-900 pl-9 pr-4 font-display text-lg tracking-widest outline-none"
      >
        {options.map((option) => (
...
function KdsLane(props: {
  ...
  onDelay: (order: Order) => void;
  onCancel: (order: Order) => void;
  onPrint: (order: Order, kind: "kot" | "bill") => void;
}) {
  return (
    <section
      className={`min-h-[calc(100vh-260px)] rounded-[20px] border ${props.tone} bg-zinc-950/80 p-3`}
    >
      <header className="mb-3 flex items-center justify-between px-2">
        ...
        <span className="grid h-12 min-w-12 place-items-center rounded-2xl bg-white text-2xl font-black text-black">
          {props.count}
        </span>
      </header>
      <div className="space-y-3">
        ...
        {props.orders.length === 0 && (
          <div className="grid h-48 place-items-center rounded-[20px] border border-dashed border-white/10 text-zinc-600">
    ...
    >
      <header className="flex items-start justify-between gap-3">
        ...
        <div className={`rounded-2xl px-3 py-2 text-right font-mono text-2xl font-black ${tone}`}>
          ...
          <div className="font-sans text-[10px] uppercase tracking-widest text-zinc-400">
      ...
      {order.customer.notes && (
        <div className="mt-3 rounded-2xl border border-orange-400/50 bg-orange-500/15 p-3 text-xl font-bold text-orange-200">
          NOTE: {order.customer.notes}
        </div>
      )}
      {m.delayReason && (
        <div className="mt-3 rounded-2xl border border-red-400/60 bg-red-500/15 p-3 text-xl font-bold text-red-200">
          DELAY: {m.delayReason} +{m.delayExtraMinutes || 0}m
        </div>
      )}
      ...
      <footer className="mt-4 grid grid-cols-2 gap-2 xl:grid-cols-5">
        {bulkMode ? (
          <button
            onClick={() =>
              setSelectedIds(
                selected ? selectedIds.filter((id) => id !== order.id) : [...selectedIds, order.id],
              )
            }
            className={`col-span-2 min-h-16 rounded-2xl font-display text-2xl ${selected ? "bg-yellow-400 text-black" : "bg-zinc-700 text-white"}`}
          >
          ...
          <>
            <ActionButton
              icon={Flame}
              label={order.status === "received" ? "Start" : "Cooking"}
            ...
            <button
              ...
              className="col-span-2 min-h-14 rounded-2xl border border-white/10 bg-black/40 font-display text-xl tracking-widest xl:col-span-5"
            >
              <Printer className="mr-2 inline h-5 w-5" /> PRINT KOT
            </button>
          </>
        )}
      </footer>
...
function ContextBlock({ order }: { order: Order }) {
  ...
  return (
    <div className="mt-4 grid grid-cols-2 gap-2 text-lg font-bold">
      ...
      <Info
        label="PICKUP"
        value={m.expectedPickup || `${m.prepEtaMinutes || m.etaMinutes || 20} min`}
      />
    </div>
  );
}
...
function ActionButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`min-h-16 rounded-2xl px-3 font-display text-xl tracking-widest ${color}`}
    >
...
function Badge({ label, tone }: { label: string; tone: string }) {
  return (
    <span className={`rounded-full px-3 py-1 text-xs font-black tracking-widest ${tone}`}>
      {label}
    </span>
  );
}
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-4 backdrop-blur-md">
      <div className="w-full max-w-2xl animate-[kds-pop_.55s_cubic-bezier(.2,1.4,.2,1)] rounded-[32px] border-2 border-red-500 bg-white/10 p-6 shadow-[0_0_90px_rgba(239,68,68,.6)] backdrop-blur-xl">
        <div className="text-center">
          <div className="animate-pulse font-display text-5xl tracking-widest text-red-400">
            NEW ORDER
          </div>
          <div className="mt-2 font-display text-7xl text-white">#{order.id}</div>
          <div className="mt-2 flex justify-center gap-2">
            <TypeBadge order={order} />
            <Badge label={order.paymentStatus.toUpperCase()} tone="bg-emerald-400 text-black" />
          </div>
          {queued > 1 && (
            <div className="mt-3 font-display text-2xl text-yellow-300">
        ...
        <div className="mt-6 rounded-[24px] bg-black/45 p-5">
          ...
          {order.customer.notes && (
            <div className="mt-4 rounded-2xl bg-orange-500/20 p-3 text-2xl font-black text-orange-200">
              {order.customer.notes}
            </div>
          )}
        </div>
        ...
        <button
          onClick={onReject}
          className="mt-4 min-h-14 w-full rounded-2xl border border-red-400/50 text-xl font-black text-red-200"
        >
          CANCEL ORDER
        </button>
      </div>
...
function SwipeAccept({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div ref={railRef} className="relative mt-6 h-20 rounded-full bg-zinc-950 p-2">
      <div className="absolute inset-0 grid place-items-center font-display text-3xl tracking-widest text-zinc-300">
        SWIPE TO ACCEPT
      </div>
      <div
        className="absolute inset-y-2 left-2 rounded-full bg-emerald-500/30"
        style={{ width: x + 74 }}
      />
      <button
    ...
    <div className="fixed inset-0 z-50 flex items-end bg-black/60 backdrop-blur-sm">
      <div className="w-full rounded-t-[32px] border-t border-white/10 bg-zinc-950 p-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-display text-5xl">Select Preparation Time</h2>
            <p className="text-zinc-400">Order #{order.id}</p>
          </div>
          <button
            onClick={onClose}
            className="grid h-14 w-14 place-items-center rounded-2xl bg-zinc-800"
          >
            <X className="h-7 w-7" />
          </button>
        </div>
        <div className="mt-6 grid grid-cols-3 gap-3 md:grid-cols-5">
          {PREP_OPTIONS.map((min) => (
            <button
              ...
              className="min-h-24 rounded-3xl bg-emerald-500 font-display text-4xl text-black"
            >
              {min}
          ...
          <div className="rounded-3xl bg-zinc-900 p-3">
            <input
              ...
              className="h-12 w-full rounded-xl bg-black px-3 text-center text-2xl"
            />
            <button
              onClick={() => onSelect(custom)}
              className="mt-2 h-12 w-full rounded-xl bg-yellow-400 font-display text-xl text-black"
            >
              CUSTOM
            </button>
          </div>
...
function DelayDialog({
  ...
  order: Order;
  onClose: () => void;
  onDelay: (reason: string, minutes: number) => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-4 backdrop-blur">
      <div className="w-full max-w-xl rounded-[28px] border border-red-400/50 bg-zinc-950 p-5">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-5xl text-red-300">Delay Order</h2>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          {DELAY_REASONS.map((r) => (
            <button
              key={r}
              onClick={() => setReason(r)}
              className={`min-h-16 rounded-2xl font-display text-xl ${reason === r ? "bg-red-500 text-white" : "bg-zinc-800"}`}
            >
              {r}
            </button>
          ))}
        ...
        {reason === "Custom" && (
          <input
            ...
            className="mt-3 h-14 w-full rounded-2xl bg-zinc-900 px-4 text-xl outline-none"
          />
        )}
        <div className="mt-5 grid grid-cols-4 gap-3">
          {DELAY_TIMES.map((m) => (
            <button
              ...
              className={`min-h-16 rounded-2xl font-display text-2xl ${minutes === m ? "bg-yellow-400 text-black" : "bg-zinc-800"}`}
            >
              +{m}
            </button>
          ))}
        ...
        <button
          onClick={() => onDelay(reason === "Custom" ? customReason || "Custom" : reason, minutes)}
          className="mt-5 min-h-16 w-full rounded-2xl bg-red-500 font-display text-2xl tracking-widest"
        >
          NOTIFY DELAY
        </button>
      </div>
...
function SoundSettingsPanel({
  ...
  setSettings: (s: KdsSettings) => void;
  onClose: () => void;
  onTest: () => void;
}) {
  function upload(file: File) {
    ...
    reader.onload = () =>
      setSettings({ ...settings, soundKind: "custom", customSound: String(reader.result) });
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/70 backdrop-blur">
      <aside className="h-full w-full max-w-md overflow-y-auto bg-zinc-950 p-5">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-5xl">Sound</h2>
          <button onClick={onClose}>
            <X className="h-8 w-8" />
          </button>
        </div>
        <div className="mt-5 space-y-4">
          ...
          <ToggleRow
            ...
            onChange={(v) => setSettings({ ...settings, voiceOn: v })}
          />
          <SelectRow
            label="Bell"
            value={settings.soundKind}
            options={["school", "kitchen", "restaurant", "alarm", "custom"]}
            onChange={(v) => setSettings({ ...settings, soundKind: v as SoundKind })}
          />
          <SelectRow
            label="Language"
            value={settings.language}
            options={["en-US", "hi-IN", "te-IN"]}
          ...
          <RangeRow
            ...
            max={20}
            step={1}
            onChange={(v) => setSettings({ ...settings, repeatInterval: v })}
          />
          ...
          <button
            onClick={onTest}
            className="min-h-16 w-full rounded-2xl bg-emerald-400 font-display text-2xl text-black"
          >
            <BellRing className="mr-2 inline h-6 w-6" /> TEST SOUND
          </button>
        </div>
      </aside>
    </div>
...
function ToggleRow({
  ...
  return (
    <label className="flex items-center justify-between rounded-2xl bg-zinc-900 p-4 text-xl font-bold">
      <span>{label}</span>
  ...
  return (
    <label className="block rounded-2xl bg-zinc-900 p-4">
      ...
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="mt-2 h-12 w-full rounded-xl bg-black px-3 text-xl"
      >
        {options.map((o) => (
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-4 backdrop-blur">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[28px] bg-zinc-950 p-5">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-5xl">#{order.id}</h2>
          <button onClick={onClose}>
            <X className="h-8 w-8" />
          </button>
        </div>
        <ContextBlock order={order} />
        <ul className="mt-5 divide-y divide-white/10 rounded-2xl bg-black/40 p-4 text-2xl font-black">
          {order.items.map((item) => (
            <li key={item.id} className="py-3">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <button
            onClick={() => onPrint(order, "kot")}
            className="min-h-16 rounded-2xl bg-zinc-800 font-display text-2xl"
          >
            PRINT KOT
          </button>
          <button
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center bg-black/80 p-4">
      <div className="no-print absolute right-4 top-4 flex gap-2">
        ...
        <button onClick={onDone} className="rounded-2xl bg-zinc-800 px-5 py-3 font-display text-xl">
          DONE
        </button>
      </div>
...
function playSuccess(settings: KdsSettings, audioRef: React.MutableRefObject<AudioContext | null>) {
  playBell(
    { ...settings, soundKind: "kitchen", soundOn: true, volume: Math.min(1, settings.volume) },
    audioRef,
  );
}
...
function speakOrder(order: Order, settings: KdsSettings) {
  ...
  const text = `New ${order.type} order. Order ${order.id.split("").join(" ")}. ${order.items.length} items. ${order.items
    .slice(0, 3)
    .map((i) => `${i.qty} ${i.name}`)
    .join(". ")}`;
...
function formatTimer(order: Order, now: number) {
  const sec = Math.max(
    ...
    Math.floor(
      (now -
        new Date(meta(order).startedAt || meta(order).acceptedAt || order.createdAt).getTime()) /
        1000,
    ),
  );
