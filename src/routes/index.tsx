          ...
          {banner && (
            ...
            >
              ...
              <div className={`relative flex ${heroHeight} flex-col justify-end p-3 sm:p-4`}>
                <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 rounded-full bg-black/35 px-1 backdrop-blur sm:bottom-4">
                  {visibleBanners.map((item, i) => (
                    <button
                      key={item.id}
                      onClick={() => setBannerIndex(i)}
                      className={`grid h-9 w-7 place-items-center p-1.5 sm:h-10 sm:w-8`}
                      aria-label={`Show banner ${i + 1}`}
                      aria-current={i === bannerIndex}
                    >
                      <span
                        className={`h-1.5 rounded-full transition-all sm:h-2 ${
                          i === bannerIndex ? "w-7 bg-white sm:w-9" : "w-1.5 bg-white/80 sm:w-2"
                        }`}
                      />
                    </button>
                  ))}
              ...
              if (!availability.available) {
                return (
                  <div key={category.id} title={availability.message} className={className}>
                    <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-zinc-100 text-lg font-black text-zinc-500">
                      {icon}
                    </div>
  ...
  return (
    <article className="min-w-[230px] overflow-hidden rounded-[28px] border border-white/80 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl md:min-w-0">
      <div className="relative">
        <Link to="/menu" search={{ category: undefined }} className="block">
