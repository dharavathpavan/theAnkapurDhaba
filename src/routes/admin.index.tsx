...
function Dashboard() {
  ...
  const { data: customerContent } = useQuery({
  ...
  const byStatus = ACTIVE_STATUSES.map((status) => ({
    status,
    count: pending.filter((o) => o.status === status).length,
  }));
  const byType = ["delivery", "pickup", "dinein"].map((type) => ({
    type,
    count: todays.filter((o) => o.type === type).length,
  }));
  ...
  const activeAnnouncements =
    customerContent?.announcements?.filter((item) => item.active).length ?? 0;
    ...
    <main className="px-4 py-6 md:px-6 lg:px-8">
        ...
        <div className="grid gap-6 p-5 md:p-7 xl:grid-cols-[1.3fr_0.7fr]">
          <div className="flex min-w-0 flex-col justify-between gap-8">
            <div>
              ...
              <h2 className="mt-5 max-w-3xl text-3xl font-black leading-tight md:text-5xl">
                The Ankapure Dhaba operations, orders, kitchen and customer app in one place.
              </h2>
              <p className="mt-4 max-w-2xl text-sm leading-6 text-white/65">
                Monitor today&apos;s sales, active kitchen load, delivery status, menu health,
                marketing content and staff portals from this admin dashboard.
              </p>
            </div>
          ...
          <div className="rounded-[24px] border border-white/10 bg-white/8 p-4">
            <img
              ...
              className="mx-auto h-32 w-32 rounded-[28px] border border-white/10 bg-black object-cover shadow-xl"
            />
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          detail={`${paidToday} paid orders`}
        />
        <StatCard
          icon={ShoppingBag}
          label="Orders today"
          value={todays.length.toString()}
          detail={`${avgOrderValue ? money(avgOrderValue) : "Rs 0"} average order`}
        />
        <StatCard
          icon={TrendingUp}
          label="Month revenue"
          value={money(revenueMonth)}
          detail={`${monthOrders.length} monthly orders`}
        />
        <StatCard
          icon={Users}
          label="Customers"
          value={customers.toString()}
          detail="Unique phone numbers"
        />
      </section>
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-5">
        <Panel
          ...
          action={
            <Link to="/admin/orders" className="text-sm font-bold text-red-600">
              View all
            </Link>
          }
        >
            ...
            {byType.map((item) => (
              ...
              >
                <span className="text-sm font-bold capitalize text-muted-foreground">
                  {item.type === "dinein" ? "Dine in" : item.type}
                </span>
        ...
        <Panel className="lg:col-span-2" title="Attention Needed">
          <div className="space-y-3">
            <AlertRow
              icon={AlertTriangle}
              label="Delayed active orders"
              value={delayed.length.toString()}
              tone={delayed.length ? "red" : "green"}
            />
            <AlertRow
              icon={ChefHat}
              label="Kitchen queue"
            ...
            <AlertRow
              ...
              value={pending.filter((o) => o.status === "out_for_delivery").length.toString()}
            />
            <AlertRow
              icon={Clock3}
              label="Ready for dispatch"
              value={pending.filter((o) => o.status === "ready").length.toString()}
            />
          </div>
        </Panel>
      </section>
      ...
      <section className="mt-6 grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <Panel
          title="Latest Orders"
          action={
            <Link to="/admin/billing" className="text-sm font-bold text-red-600">
              Go to billing
            </Link>
          }
        >
            ...
            <div className="overflow-hidden rounded-2xl border border-border">
              {latestOrders.map((order) => (
                <Link
                  key={order.id}
                  to="/admin/orders"
                  className="grid gap-3 border-b border-border bg-background p-4 last:border-b-0 hover:bg-primary/10 md:grid-cols-[1fr_auto]"
                >
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-black text-red-600">#{order.id}</span>
                      <StatusPill status={order.status} />
                      <span className="rounded-full bg-surface px-2 py-1 text-[11px] font-bold uppercase tracking-widest text-muted-foreground">
                        {order.type}
                      </span>
                    </div>
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {formatTime(order.createdAt)} · {order.paymentStatus.toUpperCase()} ·{" "}
                  ...
                  <div className="text-left md:text-right">
                    ...
                    <div className="text-xs font-bold text-muted-foreground">
                      {minutesSince(order.createdAt)} min ago
                    </div>
                  </div>
                </Link>
              ))}
        ...
        <Panel title="Store Setup">
          <div className="space-y-3">
            <InfoBlock
              ...
              value={`${customerContent?.store?.zoneRadiusKm ?? 8} km`}
            />
            <InfoBlock
              label="Delivery fee"
              value={money(customerContent?.store?.deliveryCharge ?? 10)}
            />
            <InfoBlock
              label="Minimum order"
              value={money(customerContent?.store?.minimumOrder ?? 199)}
            />
            <Link
              to="/admin/store"
              className="mt-2 inline-flex min-h-11 w-full items-center justify-center rounded-2xl bg-primary px-4 text-sm font-bold text-primary-foreground"
            >
              Edit store settings
            </Link>
          </div>
...
function Panel({
  ...
  return (
    <section
      className={`rounded-[24px] border border-border bg-surface p-4 shadow-sm md:p-5 ${className}`}
    >
...
function QuickAction({
  ...
  return (
    <Link
      to={to}
      className="flex min-h-12 items-center justify-between rounded-2xl bg-white px-4 text-sm font-black text-slate-950 hover:bg-red-50"
    >
...
function EmptyState({ text }: { text: string }) {
  return (
    <p className="rounded-2xl bg-background px-6 py-12 text-center text-sm font-semibold text-muted-foreground">
      {text}
    </p>
  );
}
