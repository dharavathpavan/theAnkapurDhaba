...
function CheckoutPage() {
  ...
  if (!lines.length && !successId) {
    return <div className="mx-auto max-w-md px-4 py-20 text-center"><h1 className="text-3xl font-black">No items in cart</h1><Link to="/menu" className="mt-6 inline-flex rounded-3xl bg-red-600 px-6 py-4 font-black text-white">Browse menu</Link></div>;
  }
      ...
      <form onSubmit={submit} className="mt-5 grid gap-5 lg:grid-cols-[1fr_390px]">
        <div className="space-y-5">
          <Panel title="Order type">
            <div className="grid grid-cols-3 gap-2">
              {(["delivery", "pickup", "dinein"] as OrderType[]).map((value) => (
                <button key={value} type="button" onClick={() => setType(value)} className={`min-h-13 rounded-2xl font-black capitalize ${type === value ? "bg-red-600 text-white" : "bg-zinc-100 text-zinc-600"}`}>{value === "dinein" ? "Dine-in" : value}</button>
              ))}
            </div>
          </Panel>
            ...
            <Panel title="Delivery address">
              {addresses.length > 0 && (
                <div className="mb-3 grid gap-2">
                  {addresses.map((item) => (
                    <button key={item.id} type="button" onClick={() => setSelectedAddress(item.id)} className={`rounded-2xl p-4 text-left ${selectedAddress === item.id || (!selectedAddress && item.isDefault) ? "bg-red-50 ring-2 ring-red-500" : "bg-zinc-100"}`}>
                      <div className="flex items-center gap-2 font-black"><Home className="h-4 w-4" /> {item.label}</div>
          ...
          <Panel title="Order notes">
            <textarea name="notes" rows={3} placeholder="Delivery notes or kitchen request" className="w-full rounded-3xl border border-zinc-200 bg-zinc-50 p-4 outline-none focus:border-red-400" />
          </Panel>
        </div>
        ...
        <aside className="h-fit rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-zinc-100">
          ...
          {coupons.length > 0 && (
            <div className="mb-4 space-y-2">
              {coupons.slice(0, 2).map((coupon) => (
                <button key={coupon.id} type="button" onClick={() => applyCoupon(coupon.code)} className="flex w-full items-center justify-between rounded-2xl border border-dashed border-red-300 bg-red-50 p-3 text-left">
                  <span><span className="font-black">{coupon.code}</span><span className="block text-xs text-zinc-500">{coupon.title}</span></span>
          ...
          <div className="flex gap-2">
            <input value={couponCode} onChange={(e) => setCouponCode(e.target.value.toUpperCase())} placeholder="Coupon code" className="min-w-0 flex-1 rounded-2xl bg-zinc-100 px-3 outline-none" />
            <button type="button" onClick={() => applyCoupon()} className="rounded-2xl bg-zinc-950 px-4 font-black text-white">Apply</button>
          </div>
      ...
      {successId && (
        <div className="fixed inset-0 z-[80] grid place-items-center bg-[#F8F9FB] p-5">
          <div className="max-w-md text-center">
            <div className="mx-auto grid h-28 w-28 place-items-center rounded-[36px] bg-green-100"><CheckCircle2 className="h-16 w-16 text-green-600" /></div>
            <h2 className="mt-6 text-4xl font-black">Order placed</h2>
            <p className="mt-2 text-zinc-500">Order #{successId} is sent to the kitchen. ETA {home?.store.averageDeliveryMin ?? 30} min.</p>
            <div className="mt-8 grid gap-3">
              <Link to="/orders/$orderId" params={{ orderId: successId }} className="rounded-3xl bg-red-600 px-6 py-4 font-black text-white">View order status</Link>
              <Link to="/menu" className="rounded-3xl bg-white px-6 py-4 font-black text-zinc-700">Continue shopping</Link>
            </div>
          </div>
        </div>
      )}
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-zinc-100"><h2 className="mb-4 text-xl font-black">{title}</h2>{children}</section>;
}

function Field({ name, label, defaultValue, placeholder, inputMode }: { name: string; label: string; defaultValue?: string; placeholder?: string; inputMode?: "tel" | "text" }) {
  return <label className="block"><span className="mb-1 block text-sm font-black">{label}</span><input name={name} defaultValue={defaultValue} placeholder={placeholder} inputMode={inputMode} className="h-13 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 outline-none focus:border-red-400" /></label>;
}
